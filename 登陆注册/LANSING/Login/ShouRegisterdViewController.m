...
- (void)textFieldDidEndEditing:(UITextField *)textField{
    if ([GCUtil isMobileNumber:numberTextField.text] != 0&&validationTextView.text.length == 6) {
        [confirmButton setBackgroundColor:RGBACOLOR(37, 182, 177, 1)];
...
-(void)theCountdown:(NSTimer *)timer{
    if (timeInt < 1) {
        [timer invalidate];
        [validationButton setBackgroundColor:RGBACOLOR(37, 182, 177, 1)];
...
//[setUpTheButton setBackgroundColor:RGBACOLOR(37, 182, 177, 1)];
//}else
    ...
    if (dict) {
        if ([[dict objectForKey:@"code"] intValue] == 1) {
            ...
            switch (aRequest.tag) {
                ...
                case 104:{
//                    LoginPublicObject *lolinPublic = [LoginPublicObject Instance];
