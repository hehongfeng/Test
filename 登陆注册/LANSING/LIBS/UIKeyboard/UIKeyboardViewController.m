...
- (void)animateView:(BOOL)isShow textField:(id)textField heightforkeyboard:(CGFloat)kheight {
	...
	if (isShow) {
		if ([textField isKindOfClass:[UITextField class]]) {
			...
			if (rect.size.height - textPoint.y < kheight) 
				rect.origin.y = rect.size.height - textPoint.y - kheight + viewFrameY-13;
			else rect.origin.y = viewFrameY;
		}
	}
	else rect.origin.y = viewFrameY;
...
-(void)textViewDidBeginEditing:(UITextView *)textView{
    if ([self.boardDelegate respondsToSelector:@selector(alttextviewBeginEditing:)]) {
		[self.boardDelegate alttextviewBeginEditing:textView];
	}
}

-(void)textViewDidChange:(UITextView *)textView{
    if ([self.boardDelegate respondsToSelector:@selector(alttextviewDidChangeEditing:)]) {
		[self.boardDelegate alttextviewDidChangeEditing:textView];
	}
}
