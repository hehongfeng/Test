...
- (CGFloat) kerningForCharacter:(NSString *)currentCharacter afterCharacter:(NSString *)previousCharacter
{
    CGFloat totalSize = [[NSString stringWithFormat:@"%@%@", previousCharacter, currentCharacter] sizeWithAttributes:self.textAttributes].width;
    CGFloat currentCharacterSize = [currentCharacter sizeWithAttributes:self.textAttributes].width;
    CGFloat previousCharacterSize = [previousCharacter sizeWithAttributes:self.textAttributes].width;
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGFloat radius = (self.radius <=0) ? (self.bounds.size.width <= self.bounds.size.height) ? self.bounds.size.width / 2 - stringSize.height: self.bounds.size.height / 2 - stringSize.height : self.radius;
    
    //Calculate the angle per charater.
    self.characterSpacing = (self.characterSpacing > 0) ? self.characterSpacing : 1;
    CGFloat circumference = 2 * radius * M_PI;
    CGFloat anglePerPixel = M_PI * 2 / circumference * self.characterSpacing;
...
//        NSString *currentCharacter = [NSString stringWithFormat:@"%c", [self.text characterAtIndex:charIdx]];
        ...
        CGFloat kerning = (lastCharacter) ? [self kerningForCharacter:currentCharacter afterCharacter:lastCharacter] : 0;
        ...
        CGFloat angle = characterPosition * anglePerPixel + startAngle;
...
- (void)setBaseAngle:(CGFloat)baseAngle
{
