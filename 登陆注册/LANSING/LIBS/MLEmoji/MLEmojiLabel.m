...
const CGFloat kLineSpacing = 6.0;
...
//    [self initPlist];
    ...
    if (self.tag == 1) {
        ...
        self.textColor = RGBACOLOR(135, 135, 135, 1);
    }else if (self.tag == 2) {
        self.font = [UIFont systemFontOfSize:15.0];
        self.lineSpacing = 8.0; //默认行间距
        self.textColor = RGBACOLOR(28, 28, 28, 1);
    }else {
        self.font = [UIFont systemFontOfSize:13.0];
        self.lineSpacing = kLineSpacing; //默认行间距
...
#pragma mark 设置链接颜色
    ...
    UIColor *commonLinkColor = self.textColor;
    //点击时候的背景色
...
//    }
     ...
     [kSlashEmojiRegularExpression() enumerateMatchesInString:testString options:0 range:NSMakeRange(0, testString.length) usingBlock:^(NSTextCheckingResult *result, __unused NSMatchingFlags flags, __unused BOOL *stop) {
         [testArray addObject:[testString substringWithRange:result.range]];
         [testDict setObject:[NSString stringWithFormat:@"Expression_%ld",(unsigned long)testArray.count] forKey:[testString substringWithRange:result.range]];
     }];
     NSString *documentDir = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
     NSString *doc = [NSString stringWithFormat:@"%@/expression.plist",documentDir];
