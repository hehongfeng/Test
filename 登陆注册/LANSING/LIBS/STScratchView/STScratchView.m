...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if ((previousTouchLocation.x > self.rect.origin.x)&&(previousTouchLocation.x < self.rect.size.width+self.rect.origin.x)&&(previousTouchLocation.y > self.rect.origin.y)&&(previousTouchLocation.y < self.rect.size.height+self.rect.origin.y)) {
        ++self.scrapeInt;
        if ((self.scrapeInt > 10)&&(self.scrapeBool == NO)) {
            self.scrapeBool = YES;
            [self.delegate EndScratch];
        }
    }
    if (self.StartBool == YES) {
        [self.delegate StartScratch];
        self.StartBool = NO;
    }
